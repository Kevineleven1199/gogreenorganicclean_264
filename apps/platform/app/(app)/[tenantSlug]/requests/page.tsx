...
type RequestsPageProps = TenantPageProps;

const RequestsPage = async ({ params }: RequestsPageProps) => {
    ...
    <div className="space-y-6">
      <header className="rounded-3xl border border-white/10 bg-white/5 p-6 text-white">
        ...
        <div className="mt-4 flex flex-wrap gap-3 text-xs text-white/60">
          <span className="rounded-full border border-white/20 px-3 py-1">Tenant: {tenantSlug}</span>
          <span className="rounded-full border border-white/20 px-3 py-1">Automation: Hybrid AI + manual review</span>
        </div>
      </header>
      <RequestTable />
    </div>
