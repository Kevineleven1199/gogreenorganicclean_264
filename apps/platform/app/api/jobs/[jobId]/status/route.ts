...
const notifyStakeholders = async ({
  ...
  if (status === "EN_ROUTE") {
    if (customerPhone) {
      await sendSms({
        to: customerPhone,
        text: `${cleanerName} is on the way for your Go Green clean at ${addressLabel}.`
      });
    }
    ...
    if (customerPhone) {
      await sendSms({
        to: customerPhone,
        text: `${cleanerName} just wrapped up your Go Green visit. Let us know if anything needs extra love.`
      });
    }
