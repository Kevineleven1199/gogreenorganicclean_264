...
import { Banknote, Download, Info, Loader2, RefreshCcw, Wallet } from "lucide-react";
...
type PayoutResponse = {
  cleanerId: string;
  upcoming: PayoutItem[];
  history: (PayoutItem & { formattedCompletedAt: string | null })[];
  totals: {
...
const CleanerPayoutsPage = () => {
  ...
  const loadPayouts = async (id: string) => {
    try {
      setLoading(true);
      const response = await fetch(`/api/cleaner/payouts?cleanerId=${encodeURIComponent(id)}`, { cache: "no-store" });
      if (!response.ok) {
        const data = (await response.json()) as { error?: string };
        throw new Error(data.error ?? "Unable to load payout history.");
      }
  ...
  const handleSubmitCleanerId = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    window.localStorage.setItem("gogreen-cleaner-id", cleanerId);
    loadPayouts(cleanerId);
  };
  ...
  const monthToDateEarnings = useMemo(() => {
    ...
    const total = payouts.history.reduce((sum, payout) => {
      if (!payout.formattedCompletedAt) return sum;
      const completed = payout.formattedCompletedAt ? new Date(payout.formattedCompletedAt) : null;
      if (completed && completed.getMonth() === currentMonth) {
    ...
    <div className="space-y-6">
        ...
        <CardContent className="space-y-5">
          <form className="flex flex-col gap-3 rounded-2xl border border-brand-100/60 bg-brand-50/40 p-4 sm:flex-row" onSubmit={handleSubmitCleanerId}>
            <input
              ...
              placeholder="Your cleaner ID (e.g., CLN-204)"
              value={cleanerId}
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            ...
            <SummaryCard
              ...
              description="All payouts since joining Go Green."
              loading={loading && !payouts}
            />
          </div>
          ...
          <div className="rounded-2xl border border-brand-100 bg-brand-50/40 p-4 text-xs text-muted-foreground">
            ...
            <p>
              Connect your PayPal email in the Go Green app to receive instant payouts. Otherwise deposits default to Stripe Connect and may
      ...
      <Card className="bg-white">
        <CardHeader className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          ...
          <button
            ...
            onClick={() => cleanerId && loadPayouts(cleanerId)}
            disabled={loading}
          >
      ...
      <Card className="bg-white">
        <CardHeader className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          ...
          <button
            type="button"
            className="inline-flex min-h-[36px] items-center gap-2 rounded-full bg-accent px-4 py-2 text-xs font-semibold uppercase tracking-[0.2em] text-white transition hover:bg-brand-700 focus:outline-none focus-visible:ring-4 focus-visible:ring-brand-200"
          >
