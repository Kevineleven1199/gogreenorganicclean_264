...
const ServiceRequestForm = ({ tenantSlug = "gogreen" }: ServiceRequestFormProps) => {
  const [state, setState] = useState<ServiceRequestPayload>({
    tenantSlug,
  ...
  const updateField = <K extends keyof ServiceRequestPayload>(
    key: K,
  ...
  return (
    <form
      ...
      className="glass space-y-6 rounded-3xl p-8 text-white"
    >
        ...
        <label className="flex flex-col gap-2 text-sm text-white/80">
          ...
          <select
            ...
            onChange={(event) =>
              updateField(
                "serviceType",
                event.target.value as ServiceRequestPayload["serviceType"]
              )
            }
